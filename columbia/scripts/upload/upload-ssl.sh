...
while true; do
	...
	if [ $FILESIZE -ge $MAX_SSL_FILESIZE ]; then
	    ...
	    ./scripts/anonymization/anonymize-offline.sh $f-incoming ./pktanon_xml/from_capture_dst_anon_include_payload.xml $ANON_KEY
	    echo "Attempting to anonymize $f-outgoing"
	    ./scripts/anonymization/anonymize-offline.sh $f-outgoing ./pktanon_xml/from_capture_src_anon_include_payload.xml $ANON_KEY
